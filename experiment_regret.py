...
def run_regret_experiment_pytorch(
    ...
    if baseline_model is None:
        baseline_model=train_baseline(dataset,
        nn_params
        )
    ...
    while counter < nn_params.max_num_steps:
        ...
        if counter == 1:
            ...
            if exploration_hparams.decision_type == "counterfactual"\
                    or exploration_hparams.decision_type == "adversarial_counterfactual":
        ...
        if exploration_hparams.decision_type == "simple":
            ...
            if biased_dataset.get_size() == 0:
                ...
                if exploration_hparams.decision_type == "counterfactual":
                    proposed_pos = torch.rand_like(initial_biased_pred) < exploration_hparams.epsilon
                else:
                    training_batch_size=min(biased_dataset.get_size(),baseline_batch_size*100)
                    historic_X, historic_y=biased_dataset.get_batch(training_batch_size)
                    ...
                    unbiased_model, generator=adversarial_predictions(batch_X, historic_X, historic_y, baseline_batch_size,train_dataset.dimension)

                    metrics_biased.append(metrics(batch_X,batch_y,model_biased))
                    metrics_adv.append(metrics_adversarial(batch_X,batch_y,unbiased_model,generator))

                    proposed_pos=torch.round(unbiased_model(generator(batch_X)).squeeze()).bool()
...
def pseudolabel(
    ...
    pseudo_Y = torch.ones(batch_X.shape[0]).to(DEVICE)
...
def adversarial_predictions(x_test,
    ...
    unbiased_model = Classifier(params['encoded_dim'],params['hidden_layer'])
    generator = Generator(dataset_dimension, params['encoded_dim'])
    discriminator = Discriminator(params['encoded_dim'], params['hidden_layer'], params['adv_loss_type'])
    generator.to(DEVICE)
    unbiased_model.to(DEVICE)

    unbiased_model, generator, discriminator = \
    adversarial_training(classifier=unbiased_model,
          generator=generator,
    ...
    return unbiased_model, generator
